...
from pyagc.encoders import create_tuned_gnn, TabularEncoder, TabularGraphEncoder
...
def build_tabular_graph_encoder(data, conf):
    """Build a TabularEncoder + graph encoder stack for TensorFrame inputs."""
    tf = data.x

    if not hasattr(tf, 'col_names_dict'):
        raise ValueError("TensorFrame is expected to expose `col_names_dict`.")
    if not hasattr(data, 'tf_col_stats'):
        raise ValueError("Data is expected to contain `tf_col_stats` for TensorFrame encoding.")

    tabular_channels = conf.get('tabular_channels', conf.get('hidden_channels', 128))
    torch_frame_model_name = conf.get('torch_frame_model', 'resnet').lower()

    # Currently only ResNet is wired here. Add more model classes if needed.
    if torch_frame_model_name != 'resnet':
        raise ValueError(f"Unsupported torch-frame model: {torch_frame_model_name}")

    tabular_encoder = TabularEncoder(
        channels=tabular_channels,
        col_names_dict=tf.col_names_dict,
        col_stats=data.tf_col_stats,
        torch_frame_model_cls=torch_frame.nn.models.ResNet,
        torch_frame_model_kwargs={
            'channels': conf.get('tf_channels', 128),
            'num_layers': conf.get('tf_num_layers', 2),
            'dropout_prob': conf.get('tf_dropout', 0.0),
        },
    )

    graph_encoder = create_tuned_gnn(
        gnn_type=conf.get('gnn_type', 'sage').lower(),
        in_channels=tabular_channels,
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='HM',
                        choices=['HM', 'Pokec', 'WebTopic'],
                        help='Dataset name')
    ...
    # Load configuration
    conf = get_training_config(args.dataset, config_path='tabular.train.conf.yaml')
    ...
    data = get_tabular_graphland_dataset(args.dataset, root=args.root, split='TH')
    y = data.y
    ...
    # Create encoder
    encoder = build_tabular_graph_encoder(data, conf)
