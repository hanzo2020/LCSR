...
class LCSR(NS4GC):
    r"""Independent LCSR model built directly on top of the NS4GC backbone.

    The message-passing graph remains the original observed graph. LCSR adds
    rectified positive-pair supervision and batch-local admission logic without
    changing the NS4GC backbone itself.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        positive_margin = kwargs.get("fcrs_positive_margin", 0.8)
        positive_temperature = kwargs.get("fcrs_positive_temperature", 0.05)
        positive_quantile = kwargs.get("fcrs_positive_quantile", 0.2)
        saturation_tau = kwargs.get("fcrs_saturation_tau", 0.80)
        saturation_temp = kwargs.get("fcrs_saturation_temp", 0.03)
        self.fcrs_positive_margin = float(0.8 if positive_margin is None else positive_margin)
        self.fcrs_positive_temperature = float(0.05 if positive_temperature is None else positive_temperature)
        self.fcrs_positive_quantile = float(0.2 if positive_quantile is None else positive_quantile)
        self.fcrs_saturation_tau = float(0.80 if saturation_tau is None else saturation_tau)
        self.fcrs_saturation_temp = float(0.03 if saturation_temp is None else saturation_temp)
