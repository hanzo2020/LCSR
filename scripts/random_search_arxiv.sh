...
for trial in $(seq 1 "$TRIALS"); do
  ...
  param_desc="lambda=$lambda warmup=$warmup rho=$rho kmax=$kmax pool=$pool margin=$margin bank_size=$bank_size batch_local_semantics=candidate_bank_v2 force_batch_local=true"
  cmd_str="$(build_command "$lambda" "$warmup" "$rho" "$kmax" "$pool" "$margin" "$bank_size")"
