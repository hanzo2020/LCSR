...
LAMBDAS=(0.002 0.005 0.01 0.02 0.03 0.05)
WARMUPS=(0 25 50 75)
RHOS=(0.05 0.10 0.15 0.20 0.25 0.30)
KMAXS=(1 2 3 4 5)
POOLS=(8 16 24 32 48 64)
...
for trial in $(seq 1 "$TRIALS"); do
  trial_name=$(printf "cora_%03d" "$trial")
  ...
  param_desc="lambda=$lambda warmup=$warmup rho=$rho kmax=$kmax pool=$pool margin=$margin gnn_type=$gnn_type hidden_channels=$hidden_channels num_layers=$num_layers epochs=$epochs p_fm1=$p_fm1 p_ed1=$p_ed1 p_fm2=$p_fm2 p_ed2=$p_ed2"
  cmd_str="$(build_command "$lambda" "$warmup" "$rho" "$kmax" "$pool" "$margin" "$gnn_type" "$hidden_channels" "$num_layers" "$epochs" "$p_fm1" "$p_ed1" "$p_fm2" "$p_ed2" "$trial_ckpt_dir")"
