...
for trial in $(seq 1 "$TRIALS"); do
  ...
  param_desc="lambda=$lambda warmup=$warmup rho=$rho kmax=$kmax pool=$pool"
  if [[ "$MARGIN_ENABLED" -eq 1 ]]; then
    param_desc="$param_desc margin=$margin"
    cmd_str="$(build_command "$lambda" "$warmup" "$rho" "$kmax" "$pool" "$margin")"
  else
    cmd_str="$(build_command "$lambda" "$warmup" "$rho" "$kmax" "$pool")"
  fi
